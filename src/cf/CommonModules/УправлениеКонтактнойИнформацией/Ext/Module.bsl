...
Процедура ПриСозданииНаСервере(Форма, Объект, ДополнительныеПараметры = Неопределено, УдалитьПоложениеЗаголовкаКИ = "",
	...
	Если ИспользуетсяКонтактнаяИнформация = Ложь Тогда
		ПараметрыВыводаКонтактнойИнформации = ПараметрыВыводаКонтактнойИнформации(Форма, ИмяЭлементаДляРазмещения, ПоложениеЗаголовкаКИ,
			ОтложеннаяИнициализация, ИсключаемыеВиды, СкрываемыеВиды);
		
		СкрытьКонтактнуюИнформацию(Форма, ДобавляемыеРеквизиты, ПараметрыВыводаКонтактнойИнформации, СсылкаНаОбъект);
		Возврат;
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Форма, Объект, Отказ = Ложь) Экспорт
	...
	Для каждого ПараметрКонтактнойИнформации Из Форма.ПараметрыКонтактнойИнформации Цикл
		...
		Если ПараметрКонтактнойИнформации.Значение.Владелец = Объект.Ссылка Тогда
	...
	Если ПараметрыКонтактнойИнформации.СкрываемыеВиды.Количество() = 0 Тогда
		Объект.КонтактнаяИнформация.Очистить();
	Иначе
		...
		Пока Индекс >= 0 Цикл
			...
			Если ПараметрыКонтактнойИнформации.СкрываемыеВиды.Найти(СтрокаТаблицы.Вид) = Неопределено Тогда
...
Процедура СкрытьКонтактнуюИнформацию(Знач Форма, Знач ДобавляемыеРеквизиты, ПараметрыКонтактнойИнформации, Знач СсылкаНаОбъект)
	...
	Если Не ПустаяСтрока(ПараметрыКонтактнойИнформации.ИмяЭлементаДляРазмещения) Тогда
		Форма.Элементы[ПараметрыКонтактнойИнформации.ИмяЭлементаДляРазмещения].Видимость = Ложь;
	КонецЕсли;
